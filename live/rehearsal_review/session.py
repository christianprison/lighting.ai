...
class Session:
    ...
    recording_started_at: Optional[datetime] = None  # wall-clock start of recording
...
def load_session(jsonl_path: Path, db: Optional[dict] = None) -> Session:
    ...
    for ev in events:
        if ev.type == "session_start":
            ...
            # started_at written by recorder since next version
            sa = ev.data.get("started_at")
            if sa:
                try:
                    recording_started_at = datetime.fromisoformat(sa)
                except ValueError:
                    pass
    ...
    # Last fallback: file modification time of the JSONL
    if recording_started_at is None:
        recording_started_at = datetime.fromtimestamp(jsonl_path.stat().st_mtime)
